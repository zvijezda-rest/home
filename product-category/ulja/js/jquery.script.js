...
jQuery(document).ready(function() { 
	...
	(function ($) {
		...
		$('.home div#middle nav').insertAfter('.middle_inner .cmsms_row:eq(0)');

		$('a.stop-prop').on('click', function(e){
			e.preventDefault();
		});

		$('li.dijamant-popup-table a, a.dijamant-popup-table').on('click', function(e){
			e.preventDefault();
	...
	(function ($) { 
		...
		if ( 
			...
			$('.cmsms_gallery, .blog.timeline').waypoint(function (dir) { 
				if (dir === 'down') {
					var el = 			$(this), 
			...
			$('.cmsms_gallery, .blog.timeline').find('article.post, .cmsms_gallery_item').addClass('shortcode_animated');
		}

		$('.blog.columns').find('article.post, .cmsms_gallery_item').addClass('shortcode_animated');
	} )(jQuery);
...
function popitup(url) {
	newwindow=window.open(url,'name', 'width=1024, height=600, scrollbars=yes');
